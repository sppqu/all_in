                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form action="{{ route('pos.destroy', $pos->pos_id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-danger btn-sm btn-delete" data-pos-name="{{ addslashes($pos->pos_name) }}"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </td>
                                </tr>
                                @empty
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content border-0 shadow">
      <div class="modal-header bg-danger text-white border-0">
        ...
        <button type="button" class="close text-white" onclick="closeDeletePosModal()" aria-label="Close" style="opacity: 1; font-size: 1.5rem; padding: 0; margin-left: 0.5rem; line-height: 1;">
      ...
      <div class="modal-footer border-0 justify-content-center">
        <button type="button" class="btn btn-secondary px-4" onclick="closeDeletePosModal()">
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-delete').on('click', function(e) {
            e.preventDefault();
            const posName = $(this).data('pos-name');
            formToDelete = $(this).closest('form');
            $('#modalPosName').text(posName);
